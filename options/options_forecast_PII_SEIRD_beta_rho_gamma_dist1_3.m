...
cadfilename1='curve-PII-SEIRD-dist1-3';

caddisease='SEIRD'; % string indicating the name of the disease related to the time series data
...
model.fc=@SEIRD1; % name of the model function
model.name='SEIRD-PII-beta-rho-gamma-dist1-3';   % string indicating the name of the ODE model

params.label={'\beta_0','\kappa','\rho','\gamma','N'};  % list of symbols to refer to the model parameters
params.LB=[0.001 0.001 0 0.01 100000]; % lower bound values of the parameter estimates
params.UB=[25 5 1 5 100000]; % upper bound values of the parameter estimates
params.initial=[0.5 1/2 0.8 1/4 100000]; % initial parameter values/guesses
params.fixed=[0 1 0 0 1]; % Boolean vector to indicate any parameters that should remain fixed (1) to initial values indicated in params.initial. Otherwise the parameter is estimated (0).
...
params.composite=@R0s_plain_unreported;  % Estimate a composite function of the individual model parameter estimates otherwise it is left empty.
params.composite_name='R_0'; % Name of the composite parameter
params.extra0='';

vars.label={'S','E','I','R','D'}; % list of symbols to refer to the variables included in the model
vars.initial=[params.initial(5)-5 0 5 0 5];  % vector of initial conditions for the model variables
vars.fit_index=[5]; %[3 4 5] % index of the model's variable that will be fit to the observed time series data
