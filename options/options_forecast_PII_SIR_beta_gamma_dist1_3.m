...
cadfilename1='curve-PII-SIR-dist1-1'; % Name of the data file (without extension) containing the incidence curve

caddisease='SIR'; % string indicating the name of the disease related to the time series data
...
% <============================== ODE model =====================================>
% <==============================================================================>
model.fc=@SIR1; % name of the model function
model.name='SIR-PII-beta-gamma-dist1-3';   % string indicating the name of the ODE model

params.label={'\beta','\gamma','N'};  % list of symbols to refer to the model parameters
params.LB=[0.001 0.1 500000]; % lower bound values of the parameter estimates
params.UB=[10 5 500000]; % upper bound values of the parameter estimates
params.initial=[0.5 1/4 500000]; % initial parameter values/guesses

params.fixed=[0 0 1]; % Boolean vector to indicate any parameters that should remain fixed (1) to initial values indicated in params.initial. Otherwise the parameter is estimated (0).
...
params.composite=@R0s_SIR;  % Estimate a composite function of the individual model parameter estimates otherwise it is left empty.
params.composite_name='R_0'; % Name of the composite parameter
params.extra0=[];

vars.label={'S','I','R','C'}; % list of symbols to refer to the variables included in the model
vars.initial=[params.initial(3)-4 4 0 4];  % vector of initial conditions for the model variables
vars.fit_index=4; % index of the model's variable that will be fit to the observed time series data
