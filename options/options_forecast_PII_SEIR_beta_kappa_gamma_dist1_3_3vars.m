...
cadfilename1='curve-PII-SEIR-dist1-3-3vars'; % Name of the data file (without extension) containing the incidence curve

caddisease='SEIR'; % string indicating the name of the disease related to the time series data
...
model.name='SEIR-PII-beta-kappa-gamma-dist1-3-3vars';   % string indicating the name of the ODE model

params.label={'\beta','\kappa','\gamma','N'};  % list of symbols to refer to the model parameters
params.LB=[0.001 0 0.1 500000]; % lower bound values of the parameter estimates
params.UB=[10 5 5 500000]; % upper bound values of the parameter estimates
params.initial=[0.5 1/2 1/4 500000]; % initial parameter values/guesses

params.fixed=[0 0 0 1]; % Boolean vector to indicate any parameters that should remain fixed (1) to initial values indicated in params.initial. Otherwise the parameter is estimated (0).
...
vars.fit_index=[3 4 5]; % index of the model's variable that will be fit to the observed time series data
vars.fit_diff=[0 0 1]; % boolean variable to indicate if the derivative of model's fitting variable should be fit to data.
