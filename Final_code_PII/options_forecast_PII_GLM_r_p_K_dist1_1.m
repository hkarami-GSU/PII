...
cadfilename1='curve-PII-GLM-dist1-1'; % Name of the data file (without extension) containing the incidence curve

caddisease='GLM'; % string indicating the name of the disease related to the time series data
...
model.fc=@GLM;  % name of the model function
model.name='GLM-PII-r-p-K-dist1-1'; % string indicating the name of the ODE model

params.num=3; % number of model parameters
params.label={'r','p','K'}; % list of symbols to refer to the model parameters
params.LB=[0.01 0.01 10];  % lower bound values of the parameter estimates
params.UB=[25 1 1000000]; % upper bound values of the parameter estimates
params.initial=[0.16 0.98 20000]; % initial parameter values/guesses
params.fixed=[0 0 0]; % Boolean vector to indicate any parameters that should remain fixed (1) to initial values indicated in params.initial. Otherwise the parameter is estimated (0).
...
params.extra0=[];  % Estimate a composite function of the individual model parameter estimates otherwise it is left empty.
...
vars.initial=6; % vector of initial conditions for the model variables
